...
This is the custom sign in by blade<br>
...
<c:choose>
	<c:when test="<%= themeDisplay.isSignedIn() %>">
		...
		<liferay-ui:message arguments="<%= signedInAs %>" key="you-are-signed-in-as : Test Test" translateArguments="<%= false %>" />
	</c:when>
	<c:otherwise>
		...
		<aui:form action="<%= loginURL %>" autocomplete='<%= PropsValues.COMPANY_SECURITY_LOGIN_FORM_AUTOCOMPLETE ? "on" : "off" %>' cssClass="sign-in-form" method="post" name="<%= formName %>" onSubmit="event.preventDefault();">
			...
			<aui:button-row>
				...
				<aui:button cssClass="btn-lg" type="reset" value="Reset" />
			</aui:button-row>
		</aui:form>
